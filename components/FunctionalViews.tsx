...
import { Search, ShieldAlert, FileText, Download, Globe, Server } from 'lucide-react';
...
export const VulnerabilitiesView: React.FC<FunctionalViewProps> = ({ missions, lang }) => {
  const t = translations[lang];
  const allVulns = missions.flatMap(m => m.vulnerabilities.map(v => ({ ...v, target: m.target })));
                ...
                <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300">
                    <thead className="bg-slate-50 dark:bg-slate-900/50 uppercase text-xs font-semibold text-slate-500 dark:text-slate-400">
                        <tr>
                            ...
                            <th className="px-6 py-4">{t.targetSystem}</th>
                    ...
                    <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                        {allVulns.map((v, idx) => (
                            <tr key={`${v.id}-${idx}`} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                <td className="px-6 py-4 font-medium text-slate-900 dark:text-white">{v.title}</td>
                                <td className="px-6 py-4 text-slate-500 dark:text-slate-400 font-mono text-xs">{v.target}</td>
