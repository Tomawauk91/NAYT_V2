        ...
        {[
          ...
          { id: 'auto', label: 'Auto-Pilot' },
            { id: 'custom', label: t.customCLI || 'Custom CLI' },
                        { id: 'filescan', label: t.fileScanTab || 'VirusTotal' },
                        ...
                        {[
                            {
                                ...
                                tools: [
                                    ...
                                    { name: 'Amass', label: "Amass", desc: "Subdomain Enum", cmd: isIpv4(mission.target) ? `dig -x ${mission.target}` : `amass enum -d ${mission.target}`, color: 'group-hover:text-blue-500' },
                                    { name: 'TheHarvester', label: "TheHarvester", desc: "OSINT Gathering", cmd: isIpv4(mission.target) ? `whois ${mission.target}` : `theHarvester -d ${mission.target} -b all`, color: 'group-hover:text-blue-500' },
                                    { name: 'DNSRecon', label: t.dnsreconBtn, desc: t.dnsreconDesc, cmd: isIpv4(mission.target) ? `dnsrecon -r ${mission.target}-${mission.target} -d local -n 127.0.0.11` : `dnsrecon -d ${mission.target}`, color: 'group-hover:text-blue-500' },
                                    { name: 'Whois', label: t.whoisBtn, desc: t.whoisDesc, cmd: `whois ${mission.target}`, color: 'group-hover:text-blue-500' },
                                    { name: 'Dig', label: t.digBtn, desc: t.digDesc, cmd: isIpv4(mission.target) ? `dig -x ${mission.target}` : `dig ${mission.target} +trace`, color: 'group-hover:text-blue-500' },
                                    ...
                                    { name: 'TShark', label: "Wireshark (CLI)", desc: "Capture 10s Traffic", cmd: `tshark -i eth0 -a duration:10 host ${mission.target}`, color: 'group-hover:text-blue-500' },
                            ...
                            },
                            {
                                ...
                                tools: [
                                    ...
                                    { name: 'ZAP', label: "OWASP ZAP", desc: "Web Scanner", cmd: `zaproxy -cmd -dir /tmp/.ZAP_${mission.id} -quickurl http://${mission.target}${targetPort ? ':' + targetPort : ''}`, color: 'group-hover:text-blue-500' },
                                    ...
                                    { name: 'TestSSL', label: t.testsslBtn, desc: t.testsslDesc, cmd: `testssl.sh --fast ${mission.target}${targetPort ? ':' + targetPort : ''}`, color: 'group-hover:text-blue-500' },
                            ...
                            },
                            {
                                ...
                                tools: [
                                    ...
                                    { name: 'FTP', label: t.ftpBtn, desc: t.ftpDesc, cmd: `nmap --script ftp-anon -p 21 ${mission.target}`, color: 'group-hover:text-blue-500' },
                            ...
                            },
                            {
                                ...
                                tools: [
                                    { name: 'Metasploit PortScan', label: "Metasploit", desc: "Auxiliary TCP Scan", cmd: `msfconsole -q -x "use auxiliary/scanner/portscan/tcp; set RHOSTS ${mission.target}; run -j; sleep 5; exit -y"`, color: 'group-hover:text-red-500' },
                                    { name: 'Hydra Brute Force', label: t.hydraBtn, desc: t.hydraDesc, cmd: `hydra -L /usr/share/seclists/Usernames/xato-net-10-million-usernames.txt -P /usr/share/wordlists/rockyou.txt ftp://${mission.target}${targetPort ? ':' + targetPort : ''}${targetPort ? '' : ':21'}`, color: 'group-hover:text-red-500' },
                                    ...
                                    { name: 'Aircrack-ng', label: "Aircrack", desc: "Speed Benchmark", cmd: `aircrack-ng -S`, color: 'group-hover:text-blue-500' },
          ...
          <div className="space-y-6 animate-fadeIn">
            <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 transition-colors">
              <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2 flex items-center gap-2">
                <Search size={20} className="text-violet-500" />
                                {t.fileScanTitle || 'VirusTotal Scan'}
              </h3>
              <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
                                {t.fileScanDesc || 'Upload a file to scan it with VirusTotal and store findings in this mission.'}
              </p>
                        ...
                        try {
                          const status = await toolsService.getScanStatus(task_id);
                                                    if (status.status === 'PROGRESS') {
                                                        const current = status.result?.output ?? '';
                                                        if (current && current !== lastProgressOutput) {
                                                            const newPart = current.startsWith(lastProgressOutput)
                                                                ? current.substring(lastProgressOutput.length)
                                                                : current;
                                                            const lines = newPart.split('\n').filter((l: string) => l.trim() !== '');
                                                            if (lines.length > 0) {
                                                                setFileScanOutput(prev => [...prev, ...lines]);
                                                            }
                                                            lastProgressOutput = current;
                                                        }
                                                        return;
                                                    }
                          if (status.status === 'SUCCESS') {
                            ...
                            setFileScanOutput(prev => [...prev, ...out.split('\n')]);
                                                        notify('success', 'VirusTotal scan completed.');
                          } else if (status.status === 'FAILURE') {
                            clearInterval(poll);
                            setIsFileScanRunning(false);
                            setFileScanOutput(prev => [...prev, '[!] Scan failed.']);
                                                        notify('error', 'VirusTotal scan failed.');
                          }
            ...
            {fileScanOutput.length > 0 && (
              <div className="bg-slate-950 rounded-xl border border-slate-800 p-4 font-mono text-sm max-h-80 overflow-y-auto">
                <div className="text-slate-500 mb-2"># VirusTotal Scan Output</div>
